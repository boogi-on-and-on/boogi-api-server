package boogi.apiserver.domain.post.post.repository;
...
public interface PostRepository extends JpaRepository<Post, Long>, PostRepositoryCustom {
    ...
    @Query(value = "SELECT post_id, community_id, member_id, content, deleted_at, like_count, comment_count, created_at, updated_at " +
            "FROM (SELECT *, RANK() OVER (PARTITION BY p.community_id ORDER BY p.created_at DESC) AS a " +
            "FROM post AS p " +
            "WHERE p.community_id IN (:communityIds) " +
            "AND p.deleted_at is NULL) AS rankrow " +
            "WHERE rankrow.a <= 1 " +
